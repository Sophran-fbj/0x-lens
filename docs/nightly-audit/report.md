...
2. **Zombie mock server**: a crashed suite run left a server bound to 5178; later suites' config/log calls silently hit it (stale rules, stale code). Port ownership is now checked before attribution; suites spawn their own server and `stop()` it in `finally`.
