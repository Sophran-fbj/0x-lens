...
2. **Zombie mock server**: a crashed suite run left a server bound to 5178; later suites' config/log calls silently hit it (stale rules, stale code). Suites now capture the spawned child's stderr and verify it is still running after the readiness probe — a child that died with EADDRINUSE fails the suite instead of letting config/log calls configure an impostor. Suites spawn their own server and `stop()` it in `finally`.
